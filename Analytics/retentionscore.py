...
def add_retention(total_comments, top_fan_cutoff, responses, total_replies, badge, delays_scaled, maxs, mins):
    ...
    if badge == 'newFan':
        return min_max_scaler(total_comments, maxs['total_comments'], mins['total_comments']) - min_max_scaler(responses, maxs['responses'], mins['responses']) \
        ...
        return 1.5*min_max_scaler(total_comments, maxs['total_comments'], mins['total_comments']) - min_max_scaler(responses, maxs['responses'], mins['responses']) + \
                                    delay3_scaled +  min_max_scaler(total_replies, maxs['total_replies'], mins['total_replies'])- 3*delay1_scaled
    else:
        return 0.5*min_max_scaler(total_comments, maxs['total_comments'], mins['total_comments']) - min_max_scaler(responses, maxs['responses'], mins['responses']) + \
